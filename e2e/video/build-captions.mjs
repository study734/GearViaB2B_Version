...
/** @type {{clips:string[],minDurationSec:number,maxLines:number,captions:Array<{id:string,clip:string,startSec:number,endSec:number,text:string}>}} */
...
if (timelinePath) {
  ...
  const missing = clipOrder.filter((c) => typeof timeline[c] !== 'number');
  if (missing.length) {
    console.error(`timeline JSON 에 offset 이 없는 clip: ${missing.join(', ')}`);
    process.exit(1);
  }
  const merged = (doc.captions ?? [])
    .map((c) => ({
      startSec: timeline[c.clip] + c.startSec,
      endSec: timeline[c.clip] + c.endSec,
      text: c.text,
    }))
    .sort((a, b) => a.startSec - b.startSec);
